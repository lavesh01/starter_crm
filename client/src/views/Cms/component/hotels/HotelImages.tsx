...
const ImageList = (props: ImageListProps) => {
    const { slideImg, onImageDelete } = props
    ...
    return (
        <>
            {slideImg.map((img) => (
                ...
                >
                    <img
                        className="rounded max-h-[120px] max-w-full"
                        src={img.img}
                        alt={img.name}
                    />
...
const HotelImages = (props: HotelImagesProps) => {
    const { values } = props
    ...
    const onUpload = (
        ...
        if (values.slideImg.length > 0) {
            const prevImgId = values.slideImg[values.slideImg.length - 1].id
    ...
    const handleImageDelete = (
        ...
        let slideImg = cloneDeep(values.slideImg)
        slideImg = slideImg.filter((img) => img.id !== deletedImg.id)
        form.setFieldValue(field.name, slideImg)
    }

    return (
        <AdaptableCard className='w-auto h-auto'>
            <FormItem>
                <Field name="slideImg">
